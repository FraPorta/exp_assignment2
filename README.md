...
The **Human Interaction Generator** component gives goal positions to the ball, using a SimpleActionClient and blocks until the goal is reached, then it sleeps for a random number of seconds between 7 and 10, so that the ball remains still.\
...
#### Ros Topics 
I will list only the ros topics directly related to the code that I developed (not all the ones only related to gazebo, ros_control)
